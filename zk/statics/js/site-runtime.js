(function () {
  ...
  const PATH_TEXT = {
    ...
    memory: ['第1-2天：静定训练、残像训练和脑屏激活，建立稳定成像基础', '第3-4天：从整段摄入到整页摄入，训练从脑内页面直接提取信息', '第5-6天：把图像化方法迁移到更多学习材料中', '第7天：通过陌生材料笔试和口试，检验真实记忆效果'],
    reading: ['破除逐字默读习惯', '训练整页摄入和脑内快照', '形成脑内电影或结构图', '通过复述和测试检验理解'],
    self: ['第1天：格物入门，掌握格定义五步法并完成基础通关', '第2天：攻坚深化，训练格定理四步法并完成核心任务', '第3天：知识联网，绘制知识全景图并进行阶段测评', '第4天：自证结营，错漏整理、闭卷测评和自学计划制定']
  };
  ...
  const EXTRA_BLOCKS = {
    ...
    memory: [{ title: '记忆训练重点', list: ['训练静坐、数息、残像等基础专注能力，先让孩子坐得住、看得稳', '通过烛光观想、曼陀罗内观和实物心像激活脑内成像能力', '从整段摄入逐步过渡到整页摄入，训练闭眼后的信息提取', '把能力迁移到更多真实学习材料中'] }],
    reading: [{ title: '阅读突破重点', list: ['破除逐字默读习惯，尝试绕开语音通道进行视觉加工', '训练整页摄入和脑内快照，让文字转化为画面和结构', '叙事文形成脑内电影，议论文形成结构图', '通过速读复述和限时理解反馈观察真实阅读变化'] }],
    self: [{ title: '自主训练重点', list: ['用“逐字读—问自己—问AI—做验证—一句话总结”理解新概念', '用“条件结论—反向追问—多种解释—生活实例”吃透核心规则', '训练孩子向AI提出精准问题，并验证AI答案是否可靠', '用费曼互讲、错漏记录和知识全景图检验是否真正学懂'] }]
  };
  ...
  const REPLACEMENTS = [
    ...
    ['五四学习法数学实训营', '心脑学习力自主营'], ['心脑学习力公开课', '心脑学习力自主营'],
    ['心脑学习力自主营（数学）（数学）', '心脑学习力自主营'], ['心脑学习力自主营（数学） （数学）', '心脑学习力自主营'], ['心脑学习力自主营（数学）', '心脑学习力自主营']
  ];
  ...
  const cleanCourseName = value => value ? value.replaceAll('心脑学习力自主营（数学）（数学）', '心脑学习力自主营').replaceAll('心脑学习力自主营（数学） （数学）', '心脑学习力自主营').replaceAll('心脑学习力自主营（数学）', '心脑学习力自主营') : value;
  const courseTitle = course => cleanCourseName(course.shortTitle || course.name || '');
  const courseSummary = course => course.summary || DATA.courseCopy?.[course.key]?.text || '';
  const courseTag = course => course.tag || DATA.courseCopy?.[course.key]?.tag || DATA.courseCopy?.[course.key]?.systemTag || '';
  const courseBullets = course => course.bullets || DATA.courseCopy?.[course.key]?.list || [];
  const courseTags = course => course.tags || [courseTag(course), '青少年学习力', '阶段训练', '过程反馈'].filter(Boolean);
  const courseAge = course => course.age || '青少年';
  const courseFormat = course => course.format || '阶段训练营';
  ...
  function ensureShell() {
    if (!q('#header')) document.body.insertAdjacentHTML('afterbegin', '<header id="header"></header>');
    if (!q('.footer')) document.body.insertAdjacentHTML('beforeend', '<footer class="footer"></footer>');
    if (!q('.sticky')) document.body.insertAdjacentHTML('beforeend', '<div class="sticky"></div>');
  }
  ...
  function renderHeader() {
    ...
    header.innerHTML = `<div class="nav-wrap">
      ...
      <nav class="nav-dropdowns">
        ...
        ${dropdown('课程产品', `${root}courses.html`, DATA.courses.map(course => ({ text: courseTitle(course), href: `${root}${course.href}` })), 'courses')}
  ...
  function renderFooter() {
    ...
    footer.innerHTML = `<div class="container"><div class="footer-grid">
      <div><span class="footer-brand-logo" aria-hidden="true"></span><h4 class="brand-mini">${DATA.brand.short}</h4><p>${DATA.brand.slogan || '青少年学习力成长 · 家庭教育支持 · 课程合作共创'}</p></div>
      <div><h4>关于我们</h4><a href="${root}about.html">公司简介</a><a href="${root}team.html">专家团队</a></div>
      <div><h4>课程产品</h4>${DATA.courses.map(course => `<a href="${root}${course.href}">${courseTitle(course)}</a>`).join('')}</div>
      ...
      <div><h4>联系我们</h4><p>电话：${DATA.brand.phone || '19321941587'}<br>微信：${DATA.brand.wechat || 'puzd1898'}<br>${DATA.brand.address || '广西壮族自治区北海市海城区茶亭路 29 号'}</p></div>
    </div><div class="copyright">© 2026 ${DATA.brand.full}</div></div>`;
  }
  ...
  function renderCourseCards() {
    ...
    if (!grid) return;
    grid.innerHTML = DATA.courses.map(course => `<article class="product-card reveal show"><span class="tag">${escapeHtml(courseTag(course))}</span><h3>${escapeHtml(courseTitle(course))}</h3><p>${escapeHtml(courseSummary(course))}</p>${listHtml(courseBullets(course))}<a class="more" href="${course.href}">FIND MORE</a></article>`).join('');
  }
  ...
  function renderDetailPage() {
    ...
    document.title = `${title}｜课程详情｜${DATA.brand.short}`;
    q('.detail-hero h1') && (q('.detail-hero h1').textContent = title);
    q('.detail-hero p') && (q('.detail-hero p').textContent = summary);
    const tags = q('.detail-tags');
    if (tags) tags.innerHTML = courseTags(course).map(tag => `<span>${escapeHtml(tag)}</span>`).join('');
    const crumb = q('.breadcrumb');
    if (crumb) crumb.innerHTML = `<a href="index.html">首页</a> / <a href="courses.html">课程产品</a> / ${escapeHtml(title)}`;

    const detailMain = q('.detail-main');
    if (detailMain) {
      detailMain.innerHTML = `<div class="detail-block reveal show"><h2>课程介绍</h2><p>${escapeHtml(copy.intro)}</p></div>
        <div class="detail-block reveal show"><h2>适合对象</h2>${listHtml(copy.audience || copy.targets)}</div>
        <div class="detail-block reveal show"><h2>训练路径</h2><div class="course-flow">${PATH_TEXT[key].map((text, index) => `<div class="flow-item"><b>${String(index + 1).padStart(2, '0')}</b><span>${escapeHtml(text.split('：')[0])}</span><p>${escapeHtml(text.includes('：') ? text.split('：').slice(1).join('：') : text)}</p></div>`).join('')}</div></div>
        ${EXTRA_BLOCKS[key].map(block => `<div class="detail-block reveal show"><h2>${escapeHtml(block.title)}</h2>${block.flow ? `<div class="feature-grid">${block.flow.map(([itemTitle, itemText], index) => `<div class="feature-item"><b>${String(index + 1).padStart(2, '0')}</b><h3>${escapeHtml(itemTitle)}</h3><p>${escapeHtml(itemText)}</p></div>`).join('')}</div>` : listHtml(block.list)}</div>`).join('')}
        <div class="course-challenge" data-course="${key}"><div class="challenge-head"><span>Stage Challenge</span><h2>${escapeHtml(DATA.courseChallenges[key].title)}</h2><p>${escapeHtml(DATA.courseChallenges[key].intro || DATA.courseChallenges[key].description)}</p></div><div class="challenge-grid">${DATA.courseChallenges[key].items.map(item => `<article class="challenge-card"><b>${escapeHtml(item.day || item[0])}</b><h3>${escapeHtml(item.title || item[0])}</h3><p>${escapeHtml(item.text || item[1])}</p></article>`).join('')}</div></div>`;
    }

    const side = q('.side-card');
    if (side) side.innerHTML = `<h3>课程信息</h3><p>${escapeHtml(summary)}</p><div class="side-list"><div><b>适合年龄</b><span>${escapeHtml(courseAge(course))}</span></div><div><b>课程形式</b><span>${escapeHtml(courseFormat(course))}</span></div><div><b>课程价格</b><span>${escapeHtml(course.price)}</span></div></div><a class="btn btn-gold" href="contact.html">微信咨询</a>`;
